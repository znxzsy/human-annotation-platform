...
function recheckDraftFrom(item){return Array.from({length:5},(_,i)=>{const old=(item.rechecks||[]).find(x=>x.slot===i+1&&(!isRecheckPool||x.pool===caseKind))||{};return {verdict:old.verdict||null,note:old.note||'',final_verdict:old.final_verdict||null,final_r:old.final_r||'',final_h:old.final_h,final_reason_code:old.final_reason_code||''}})}
...
async function openNextPendingRecheck(){try{sync('本组复核完成，正在自动跳转下一条…');const query=new URLSearchParams({pool:caseKind,mode:'random',pending:'1',start:String(batchStart),end:String(batchEnd)});const item=await api('api/recheck-pick?'+query);await loadItem(item)}catch(err){sync('当前批次待复核数据已完成','ok');showError('当前批次没有更多待复核数据')}}
...
function recheckComplete(d){if(d?.verdict==='accurate')return true;if(d?.verdict!=='inaccurate')return false;if(d.final_verdict==='correct')return true;if(d.final_verdict==='wrong')return Boolean(String(d.final_r||'').trim());if(d.final_verdict==='unsure')return ['image_blurred','no_handwriting','ungradable'].includes(d.final_reason_code);return false}
function correctionMarkup(d,disabled){if(d.verdict!=='inaccurate')return '';return `<div class="final-correction"><div class="final-copy"><b>请给出最终标签</b><span>完成修正后，这条数据会重新参与训练导出。</span></div><div class="final-verdicts"><button data-final-v="correct" class="${d.final_verdict==='correct'?'active correct':''}" ${disabled}>模型结果正确</button><button data-final-v="wrong" class="${d.final_verdict==='wrong'?'active wrong':''}" ${disabled}>模型结果错误</button><button data-final-v="unsure" class="${d.final_verdict==='unsure'?'active unsure':''}" ${disabled}>仍为 Unknown</button></div>${d.final_verdict==='wrong'?`<div class="final-wrong"><input data-final-r value="${esc(d.final_r)}" placeholder="填写最终修正结果" ${disabled}><button data-final-save ${disabled}>保存修正并下一条</button></div>`:''}${d.final_verdict==='unsure'?`<div class="final-reasons"><button data-final-reason="image_blurred" class="${d.final_reason_code==='image_blurred'?'active':''}" ${disabled}>图片残缺或模糊</button><button data-final-reason="no_handwriting" class="${d.final_reason_code==='no_handwriting'?'active':''}" ${disabled}>无手写作答</button><button data-final-reason="ungradable" class="${d.final_reason_code==='ungradable'?'active':''}" ${disabled}>无法判断</button></div>`:''}</div>`}
function slotRecheckMarkup(i){if(!isRecheckPool||state.item.status!=='submitted'||!recheckEligible(i))return '';const d=state.recheckDraft[i]||{verdict:null,note:''},disabled=state.recheckSaving?'disabled':'';return `<div class="slot-recheck"><div class="slot-recheck-copy"><b>SLOT ${i+1} 二次复核</b><span>点“准确”会自动保存并进入下一条；点“不准”后请完成最终修正。</span></div><div class="recheck-actions"><button data-rv="accurate" class="${d.verdict==='accurate'?'active accurate':''}" ${disabled}>人工原标准确</button><button data-rv="inaccurate" class="${d.verdict==='inaccurate'?'active inaccurate':''}" ${disabled}>发现人工标注不准</button><input data-rnote value="${esc(d.note)}" placeholder="该 SLOT 复核备注（可选）" ${disabled}>${correctionMarkup(d,disabled)}</div></div>`}
...
function bindRecheckEvents(){document.querySelectorAll('.slot').forEach(el=>{const i=+el.dataset.slot;el.querySelectorAll('[data-rv]').forEach(b=>b.onclick=async e=>{e.stopPropagation();if(state.recheckSaving)return;const previous={...state.recheckDraft[i]},value=b.dataset.rv;if(value==='inaccurate'&&previous.verdict==='inaccurate'){renderSlots();return}state.recheckDraft[i].verdict=value;Object.assign(state.recheckDraft[i],{final_verdict:null,final_r:'',final_h:null,final_reason_code:''});renderSlots();await saveRecheck(i,previous,value==='accurate')});el.querySelectorAll('[data-final-v]').forEach(b=>b.onclick=async e=>{e.stopPropagation();if(state.recheckSaving)return;const previous={...state.recheckDraft[i]},value=b.dataset.finalV;Object.assign(state.recheckDraft[i],{final_verdict:value,final_r:'',final_h:value==='unsure'?1:null,final_reason_code:''});renderSlots();if(value==='correct')await saveRecheck(i,previous,true)});const finalR=el.querySelector('[data-final-r]');if(finalR){finalR.onclick=e=>e.stopPropagation();finalR.oninput=()=>{state.recheckDraft[i].final_r=finalR.value;state.recheckDraft[i].final_h=0}}const finalSave=el.querySelector('[data-final-save]');if(finalSave)finalSave.onclick=async e=>{e.stopPropagation();const previous=recheckDraftFrom(state.item)[i];await saveRecheck(i,previous,true)};el.querySelectorAll('[data-final-reason]').forEach(b=>b.onclick=async e=>{e.stopPropagation();if(state.recheckSaving)return;const previous={...state.recheckDraft[i]};state.recheckDraft[i].final_reason_code=b.dataset.finalReason;renderSlots();await saveRecheck(i,previous,true)});const note=el.querySelector('[data-rnote]');if(note){note.onclick=e=>e.stopPropagation();note.oninput=()=>{state.recheckDraft[i].note=note.value};note.onchange=()=>setTimeout(async()=>{const saved=recheckDraftFrom(state.item)[i],current=state.recheckDraft[i];if(current.verdict&&!state.recheckSaving&&JSON.stringify(current)!==JSON.stringify(saved))await saveRecheck(i,saved,false)},300)}})}
...
async function advanceAfterRecheck(i){const eligible=state.recheckDraft.map((d,index)=>({d,index})).filter(x=>x.index!==i&&recheckEligible(x.index)&&!recheckComplete(x.d)),next=eligible.find(x=>x.index>i)||eligible[0];if(next){state.focus=next.index;renderSlots();sync(`已自动跳到 SLOT ${next.index+1}`,'ok');requestAnimationFrame(()=>document.querySelector(`.slot[data-slot="${next.index}"]`)?.scrollIntoView({behavior:'smooth',block:'start'}));return}await openNextPendingRecheck()}
async function saveRecheck(i,previous=null,autoAdvance=false){
  const d={...state.recheckDraft[i]};
  if(!d?.verdict){sync(`请选择 SLOT ${i+1} 的二次复核结论`,'offline');return false}
  if(autoAdvance&&!recheckComplete(d)){sync('请先完成最终修正再进入下一条','offline');return false}
  if(state.recheckSaving)return false;
  state.recheckSaving=true;renderSlots();let saved=false;
  try{
    sync(`正在自动保存 SLOT ${i+1} 二次复核…`);
    const item=await api(`api/items/${state.item.event_id}/recheck`,{method:'POST',body:JSON.stringify({slot:i+1,verdict:d.verdict,note:d.note,pool:caseKind,final_verdict:d.final_verdict,final_r:d.final_r,final_h:d.final_h,final_reason_code:d.final_reason_code,idempotency_key:key()})});
    state.item={...state.item,...item};state.recheckDraft=recheckDraftFrom(item);saved=true;
    sync(`SLOT ${i+1} 二次复核已自动保存`,'ok');
  }catch(err){
    if(previous)state.recheckDraft[i]={...previous};
    sync(`SLOT ${i+1} 二次复核保存失败，已恢复原状态`,'offline');showError(err.message);
  }finally{state.recheckSaving=false;renderSlots()}
  if(saved&&autoAdvance)await advanceAfterRecheck(i);
  return saved
}
